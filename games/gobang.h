...
public:
	...
	typedef struct Move
	{
		int x;
		int y;
		friend std::istream& operator>>(std::istream& in, struct Move& move){in>>move.x>>move.y;return in;}
		friend std::ostream& operator<<(std::ostream& out,const struct Move& move){out<<move.x<<' '<<move.y;return out;}
		bool operator==(const struct Move& ot)const{return (this->x==ot.x)&&(this->y==ot.y);}
		bool operator!=(const struct Move& ot)const{return !((*this)==ot);}
		bool operator<(const struct Move& ot)const{return (this->x<ot.x)||((this->x==ot.x)&&(this->y<ot.y));}
	}	Move;
	static const Move no_move;

	static const char player_markers[3]; 

	GoBangState(int num_rows_ = 6, int num_cols_ = 7)
		: player_to_move(1),
	      num_rows(num_rows_),
	      num_cols(num_cols_),
		  last_col(-1),
		  last_row(-1),
			reserved_num(num_rows_ * num_cols_)
	{ 
	...
	void do_random_move(RandomEngine* engine)
	{
		dattest(has_moves());
		check_invariant();
		std::uniform_int_distribution<int> moves(0, num_cols - 1);
	...
	std::vector<Move> get_moves() const
	{
		...
		moves.reserve(num_cols*num_rows);
...
ostream& operator << (ostream& out, const GoBangState& state)
{
	state.print(out);
	return out;
}
